...
tc_instance = os.getenv("tc_instance")
...
if not tc_instance:
    logging.error("Missing environment variable for tc_instance")
    exit(1)
...
def generate_auth_header(api_path, query_string, http_method, timestamp):
    message = (
        f"{api_path}{query_string}:{http_method}:{timestamp}"
        if query_string
        else f"{api_path}:{http_method}:{timestamp}"
    )
    signature = base64.b64encode(
        hmac.new(tc_secretkey.encode(), message.encode(), hashlib.sha256).digest()
    ).decode()
...
def query_indicator_with_tql(indicator_type: str, indicator: str):
    try:
        ...
        full_url = f"https://{tc_instance}.threatconnect.com{api_path}{query_string}"
...
def format_and_print_indicator_data(indicator_data):
    for indicator in indicator_data:
        ...
        if date_added != "N/A":
            date_added = datetime.strptime(date_added, "%Y-%m-%dT%H:%M:%SZ").strftime(
                "%B %d, %Y %H:%M:%S"
            )
        ...
        print(
            f"{Fore.RED}{Style.BRIGHT}Summary:{Style.RESET_ALL} {indicator.get('summary', 'N/A')}"
        )
        print(f"{Fore.RED}{Style.BRIGHT}Date Added:{Style.RESET_ALL} {date_added}")
        print(
            f"{Fore.RED}{Style.BRIGHT}Last Modified:{Style.RESET_ALL} {last_modified}"
        )
        print(
            f"{Fore.RED}{Style.BRIGHT}Type:{Style.RESET_ALL} {indicator.get('type', 'N/A')}"
        )
        print(
            f"{Fore.RED}{Style.BRIGHT}Rating:{Style.RESET_ALL} {'💀' * int(indicator.get('rating', 0))} ({indicator.get('rating', 'N/A')}/5.0)"
        )
        print(
            f"{Fore.RED}{Style.BRIGHT}Confidence:{Style.RESET_ALL} {indicator.get('confidence', 'N/A')}%"
        )
        print(
            f"{Fore.RED}{Style.BRIGHT}Owner:{Style.RESET_ALL} {indicator.get('ownerName', 'N/A')}"
        )
        print(
            f"{Fore.RED}{Style.BRIGHT}Active:{Style.RESET_ALL} {'Yes' if indicator.get('active', False) else 'No'}"
        )
        print(
            f"{Fore.RED}{Style.BRIGHT}Web Link:{Style.RESET_ALL} {indicator.get('webLink', 'N/A')}"
        )
        ...
        if "source" in indicator:
            print(
                f"{Fore.RED}{Style.BRIGHT}Source:{Style.RESET_ALL} {indicator.get('source', 'N/A')}"
            )
...
def main():
    ...
    for indicator in indicators:
        if indicator:  # Ensure the indicator is not empty
            ...
            print(
                Fore.YELLOW
                + f"Processing Indicator: {indicator}, Type: {indicator_type}"
            )
            data = query_indicator_with_tql(
                indicator_type, indicator
            )  # Correctly pass 'indicator' here
            if data and "data" in data and data["status"] == "Success":
                format_and_print_indicator_data(data["data"])
            else:
                print(
                    Fore.RED + "No data returned from the query or an error occurred."
                )
